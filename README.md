...
### 4. Загрузка изображений для предметов
Для загрузки логотипов и баннеров предметов используется локальное хранилище файлов в папке `public/uploads/subjects/`. Файлы загружаются через API endpoint `/api/upload` и сохраняются с уникальными именами. В базе данных хранятся только пути к файлам (например, `/uploads/subjects/logo-1234567890-abc123.png`).

**Важно**: Папка `public/uploads/` добавлена в `.gitignore`, поэтому загруженные файлы не попадут в репозиторий. На продакшн-сервере используйте CDN или облачное хранилище (Cloudinary, AWS S3).
...
- ✅ Форма генерации worksheet (2 варианта: `/generate` и `/generate-chat`)
...
📖 **См. также:** [AI_SERVICE_CLASSIFICATION.md](AI_SERVICE_CLASSIFICATION.md) - Полная классификация типов запросов учителей и план реализации
